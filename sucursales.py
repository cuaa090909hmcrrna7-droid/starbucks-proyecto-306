...
def abrir_sucursales():
    ...
    sucursales.configure(bg="#00704A")  
    campos = ["id_sucursal", "nombre_sucursal", "direccion"]
    entradas = {}

    for i, texto in enumerate(campos):
        Tk.Label(sucursales, text=texto, bg="#00C986").grid(row=i, column=0, padx=10, pady=5, sticky="w")
        entradas[texto] = Tk.Entry(sucursales, bg="#FFFFFF", fg="#000000")  
    ...
    for i, (texto, cmd) in enumerate(botones):
        Tk.Button(sucursales, text=texto, width=12, command=cmd, bg="#00704A", fg="#FFFFFF").grid(row=4, column=i, padx=10, pady=10)
    ...
    Tk.Button(sucursales, text="Regresar al Menú", width=20, command=lambda: [sucursales.destroy(), menu.abrir_menu()], bg="#00704A", fg="#FFFFFF").grid(row=7, column=0, columnspan=4, pady=10)
