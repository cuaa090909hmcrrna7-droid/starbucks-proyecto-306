...
def abrir_productos():
    ...
    productos.state('zoomed')
    productos.configure(bg="#1E3932")  # Color verde oscuro de Starbucks

    campos = ["id_productos", "nombre_productos", "precio_uni", "id_proveedor"]
    entradas = {}

    for i, texto in enumerate(campos):
        Tk.Label(productos, text=texto, bg="#00704A", fg="#FFFFFF").grid(row=i, column=0, padx=10, pady=5, sticky="w")  # Color verde más claro de Starbucks
        entradas[texto] = Tk.Entry(productos, bg="#FFFFFF", fg="#1E3932")
    ...
    for i, (texto, cmd) in enumerate(botones):
        Tk.Button(productos, text=texto, width=12, bg="#00704A", fg="#FFFFFF", command=cmd).grid(row=4, column=i, padx=10, pady=10)  # Color verde más claro de Starbucks
    ...
    Tk.Button(productos, text="Regresar al Menú", width=20, bg="#00704A", fg="#FFFFFF", command=lambda: [productos.destroy(), menu.abrir_menu()]).grid(row=7, column=0, columnspan=4, pady=10)  # Color verde más claro de Starbucks
