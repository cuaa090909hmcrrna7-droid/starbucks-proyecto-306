...
def abrir_ventas():
    ...
    for texto, fila, col in campos:
        tk.Label(ventas, text=texto, bg="#00C986").grid(row=fila, column=col, padx=10, pady=5, sticky="w")
        entrada = tk.Entry(ventas, bg="#FFFFFF", fg="#000000")                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 
    ...
    def insertar():
        if not entradas["id_ventas"].get() or not entradas["Fecha (YYYY-MM-DD)"].get() or not entradas["id_Productos"].get():
            messagebox.showwarning("Campos vacíos", "id_ventas, Fecha y Producto son obligatorios")
            return
        sql = """INSERT INTO ventas 
                 (id_ventas, fecha, id_productos, id_empleados, id_sucursal, cantidad, precio_unitario, 
                 subtotal, iva, total) 
                 VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
        params = tuple(entradas[c].get() for c in [
            "id_ventas", "Fecha (YYYY-MM-DD)", "id_Productos", "id_empleados", "id_sucursal", 
    ...
    def actualizar():
        ...
        if not seleccionado:
            ...
            return
        sql = """UPDATE ventas 
                 SET fecha=%s, id_productos=%s, id_empleados=%s, id_sucursal=%s, cantidad=%s, 
    ...
    for i, (texto, cmd) in enumerate(botones):
        tk.Button(ventas, text=texto, width=10, command=cmd, bg="#00704A", fg="#FFFFFF").grid(row=6, column=i, padx=9, pady=9)  

    columnas = ("id_ventas", "Fecha", "id_Productos", "id_empleados", "id_sucursal", "Cantidad", "Precio Unitario", "Subtotal", "IVA", "Total")
    ...
    tk.Button(ventas, text="Regresar al Menú", width=20,
              command=lambda: [ventas.destroy(), menu.abrir_menu()], bg="#00704A", fg="#FFFFFF").grid(row=8, column=0, columnspan=4, pady=10)  
